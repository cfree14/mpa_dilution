...
find_additions <- function(wdpa_ts){
  ...
  for(i in 2:length(years)){
    ...
    year_now <- years[i]
    year_last <- years[i-1]
    mpaids_now <- sort(unique(wdpa_ts$wdpaid[wdpa_ts$year==year_now]))
    mpaids_last <- sort(unique(wdpa_ts$wdpaid[wdpa_ts$year==year_last]))
    print(year_now)
...
# Export data
write.csv(mpas_changed_all, paste(datadir, "2004-17_network_change_events_wdpaid.csv", sep="/"), row.names=F)
save(mpas_lost_all, mpas_added_all, mpas_changed_all,
     file=paste(datadir, "2014-17_network_change_events_wdpaid.Rdata", sep="/"))
